#!/usr/bin/env python3
"""跑 L3 uu 取号（未安装场景），收集工具链+content → results/l3_uu_uninstalled_collection.json"""
import json
import sys
...
OUT = Path(__file__).parent.parent / "results" / "l3_uu_uninstalled_collection.json"


def run_one(tid):
    p = Path(__file__).parent.parent / "papers" / "L3" / f"{tid.split('_', 1)[1]}.yaml"
    task = yaml.safe_load(p.read_text(encoding="utf-8"))
    q = task["query"].replace("{device}", DEVICE).replace("{code}", "163a163a")
    task["query"] = q
    print(f"\n########## {tid} ##########\nquery:", q)
...
def main():
    results = []
    for tid in ("L3_uu_take_code_uninstalled",):
        try:
