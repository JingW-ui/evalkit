#!/usr/bin/env python3
"""干净顺序跑 L3 uu 三题：已安装取号 → 卸载 → 未安装取号（skill 已修 page_size）→ results/l3_uu_clean_collection.json"""
import json
import sys
...
BINDINGS = {
    ...
    "L3_uu_take_code_uninstalled": {"{device}": DEVICE, "{code}": "163a163a"},
}
OUT = Path(__file__).parent.parent / "results" / "l3_uu_clean_collection.json"
...
def run_one(tid):
    ...
    with ClaudeEvalBackend(
        ...
        r = b.run_task(task, timeout_s=900)
    m = r.get("metrics") or {}
    tools = []
...
def main():
    results = []
    for tid in ("L3_uu_take_code", "L3_uu_uninstall", "L3_uu_take_code_uninstalled"):
        try:
